...
describe("S3 — memory and peripherals", { skip: !ON_HIL_RUNNER && "requires HIL=1" }, () => {
  ...
  test("SCB CPUID identifies a Cortex-M4", async () => {
    ...
    const out = await hil.expectOk("read_memory", { address: NRF52840.CPUID_ADDR, length: 4 });
  ...
  test("FICR reports the part number", async () => {
    const out = await hil.expectOk("read_memory", { address: NRF52840.FICR_INFO_PART, length: 4 });
  ...
  test("FICR DEVICEID is stable across reads and across a reset", async () => {
    const first = await hil.expectOk("read_memory", { address: NRF52840.FICR_DEVICEID_0, length: 8 });
    record("hil-read-deviceid.txt", first);
    const second = await hil.expectOk("read_memory", { address: NRF52840.FICR_DEVICEID_0, length: 8 });
    assert.equal(word32(second, 0), word32(first, 0), "DEVICEID changed between reads");
    assert.equal(word32(second, 1), word32(first, 1));

    await hil.expectOk("reset", { halt: true });
    const third = await hil.expectOk("read_memory", { address: NRF52840.FICR_DEVICEID_0, length: 8 });
  ...
  test("write_memory round-trips little-endian", async () => {
    ...
    await hil.expectOk("write_memory", { address: NRF52840.SCRATCH, value: 0xdeadbeef });
    const out = await hil.expectOk("read_memory", { address: NRF52840.SCRATCH, length: 4 });
  ...
  test("distinct values land at distinct addresses", async () => {
    await hil.expectOk("write_memory", { address: NRF52840.SCRATCH, value: 0x11111111 });
    await hil.expectOk("write_memory", { address: NRF52840.SCRATCH + 4, value: 0x22222222 });
    const out = await hil.expectOk("read_memory", { address: NRF52840.SCRATCH, length: 8 });
  ...
  test("a multi-line dump yields exactly the bytes requested", async () => {
    ...
    const out = await hil.expectOk("read_memory", { address: 0, length: 20 });
  ...
  test("a large read stays consistent", async () => {
    const out = await hil.expectOk("read_memory", { address: 0, length: 256 });
  ...
  test("DHCSR is readable — the preflight path works", async () => {
    const out = await hil.expectOk("read_memory", { address: NRF52840.DHCSR_ADDR, length: 4 });
  ...
  test("an unmapped address fails without wedging the session", async () => {
    const out = await hil.call("read_memory", { address: NRF52840.UNMAPPED, length: 4 });
    record("hil-read-unmapped.txt", out);

    // The contract is that whatever happens, the next read still works.
    const after = await hil.expectOk("read_memory", { address: NRF52840.CPUID_ADDR, length: 4 });
